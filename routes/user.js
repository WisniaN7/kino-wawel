...
router.get('/*/bilety', async (req, res, next) => {
    ...
    res.render('tickets', { tickets: tickets, user: req.session.user, host: req.rawHeaders[1] })
})
...
router.get('/*/recenzje', async (req, res, next) => {
    ...
    res.render('reviews', { reviews: reviews, user: req.session.user, host: req.rawHeaders[1] })
})
...
router.get('/*/obejrzane', async (req, res, next) => {
    ...
    res.render('watched', { movies: movies, user: req.session.user, host: req.rawHeaders[1] })
})
...
router.get('/*/recenzje/nowa/:id', async (req, res, next) => {
    ...
    if (!await userController.userWatchedmovie(req.session.user.user_id, req.params.id)) {
    ...
    if (!movie) {
        res.render('edit review', {
            snackbar: { message: 'Wystąpił błąd podczas pobierania danych. Odśwież stronę lub skontatkuj się z administratorem serwisu.', type: 'error' },
            editMode: false, user: req.session.user, host: req.rawHeaders[1]
        })

        return
    }

    res.render('edit review', { editMode: false, movie: movie, user: req.session.user, host: req.rawHeaders[1] })
})
...
router.get('/*/recenzje/edytuj/:id', async (req, res, next) => {
    ...
    if (!review) {
        res.render('edit review', {
            snackbar: { message: 'Wystąpił błąd podczas pobierania danych. Odśwież stronę lub skontatkuj się z administratorem serwisu.', type: 'error' },
            editMode: true, user: req.session.user, host: req.rawHeaders[1]
        })

        return
    }
    
    res.render('edit review', { editMode: true, review: review, user: req.session.user, host: req.rawHeaders[1] })
})
