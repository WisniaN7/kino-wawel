...
router.get('/:title/:screening', async (req, res, next) => {
    ...
    if (!ticketTypes || !tickets) {
        res.render('purchase', {
            ...
            user: req.session.user, screening: req.params.screening, title: req.params.title, host: req.headers.host
        })

        return
    }

    res.render('purchase', { user: req.session.user, screening: req.params.screening, title: req.params.title, tickets: tickets, ticketTypes: ticketTypes, host: req.headers.host })
})
...
router.get('/sukces', (req, res, next) => {
    res.render('success', { user: req.session.user, email: req.query.email, host: req.headers.host })
})
