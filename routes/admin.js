...
router.get('/filmy/edytuj/:id/?*', async (req, res, next) => {
    ...
    res.render('edit movie', { movie: movie, cinemas: cinemas, genres: genres, user: req.session.user, host: req.headers.host, protocol: req.protocol })
})
...
router.get('/seanse/:city/*', async (req, res, next) => {
    ...
    res.render('edit screenings', { movies: movies, cinemas: cinemas, halls: halls, user: req.session.user, host: req.headers.host, protocol: req.protocol })
})
