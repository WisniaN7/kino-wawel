...
const redirectTo404 = (req, res, next) => {
    ...
    res.status(404)
    res.render('error', { user: req.session.user, host: req.headers.host })
}
...
router.get('/', async (req, res, next) => {
    ...
    res.render('admin', { movies: movies, cinemas: cinemas, user: req.session.user, host: req.headers.host })
})
...
router.get('/filmy/edytuj/:id/?*', async (req, res, next) => {
    ...
    res.render('edit movie', { movie: movie, cinemas: cinemas, genres: genres, user: req.session.user, host: req.headers.host })
})
...
router.get('/seanse/:city/*', async (req, res, next) => {
    ...
    res.render('edit screenings', { movies: movies, cinemas: cinemas, halls: halls, user: req.session.user, host: req.headers.host })
})
